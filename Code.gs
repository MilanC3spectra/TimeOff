...
function process(row) {
  ...
  let message = `Poštovani ${name}, Vaš zahtev za odmor u periodu od `
      + `${startDate.toDateString()} do `
 ...
 const docCopy = docTemplate.makeCopy(`${name}`,docFolder);
 const doc = DocumentApp.openById(docCopy.getId());
 const docBody = doc.getBody();
 docBody.replaceText('{{name}}', `${name}`);
 docBody.replaceText('{{startDate}}', `${startDate.toDateString()}`);
 docBody.replaceText('{{endDate}}', `${endDate.toDateString()}`);
 doc.saveAndClose();


  if (approval == Approval.NotApproved) {
    // If not approved, send an email.
    let subject = 'Vaš zahtev za odmor NIJE odobren';
    MailApp.sendEmail({
      to: email,
      subject: subject,
      htmlBody:message,
    ...
    let subject = 'Potvrdra, Vaš zatev za odmor je ODOBREN!';
     MailApp.sendEmail({
      to: email,
      subject: subject,
      htmlBody:message,
